...
8747
mygenesNoZSRWSR = subset(allmygenes, !(allmygenes$genes %in% all_ZW$X2))

8747-7847

write.csv(x = mygenesNoZSRWSR, file = 'allmiragenes_noZSR_noZSR.csv')
```
...
no_ZSR_sorted <- subset(run1.sorted.filt, features = mygenesNoZSRWSR$genes)
...
no_ZSR_unsorted <- subset(run1.unsorted.filt, features = mygenesNoZSRWSR$genes)
...
ggsave("run1.nozsr.nowsr.v10.integrated.elbow.pdf")
...
ggsave("run1.nozsr.nowsr.clust_55PC_0_to_1_tree_SCT.pdf")
```
...
p2
ggsave("run1.nozsr.nowsr-clust-55-PC_integrate_52_plex.pdf", width = 25, height = 15, units = c('cm'))

run1.no_ZSR <- FindClusters(run1.no_ZSR, resolution = 1)
p2 <- DimPlot(run1.no_ZSR, reduction = "umap", label = TRUE, repel = TRUE)
p2
ggsave("run1.nozsr.nowsr-clust-55-PC-1-res_integrate_52_clean.pdf")
```
...
p2 <- DimPlot(run1.no_ZSR, reduction = "umap", label = TRUE, repel = TRUE)
p2
ggsave("run1clust_55PC_1res_integrate.run1.nozsr.nowsr_maylabels.pdf")
ggsave("run1clust_55PC_1res_integrate.run1.nozsr.nowsr_maylabels_big.pdf", width = 35, height = 25, units = c('cm'))
```
...
ggsave("run1.v10.nozsr.nowsr_umap_fancyoldlabels_nov23.pdf", width = 20, height = 15, units = c('cm'))
ggsave("run1.v10.no_nozsr.nowsr_umap_fancyoldlabels_nov23_small.pdf")
```
