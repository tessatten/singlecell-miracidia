...
```{r}
d <- "/Users/ta13/R_git/miracidia_v10_scvelo/sample4_kb/counts_filtered/"
...
```{r, include=FALSE}
saveRDS(bc2, "/Users/ta13/R_git/miracidia_v10_scvelo/sample4_kb/counts_filtered/bivar_knee.rds")
```
...
```{r}
write.csv(x = seu@meta.data, file = 'sample4_kb/seu_meta_filt_miracidia_sample4.csv')
write.csv(x = t(as.matrix(seu@assays$sf@counts)), file = 'sample4_kb/seu_spliced_raw_filt_miracidia_sample4.csv')
write.csv(x = t(as.matrix(seu@assays$uf@counts)), file = 'sample4_kb/seu_unspliced_raw_filt_miracidia_sample4.csv')
```

```{r, fig.height=12, fig.width=6}
cols_use <- c("nCount_sf", "nFeature_sf", "nCount_uf", "nFeature_uf")
VlnPlot(seu, cols_use, pt.size = 0.1, ncol = 1, group.by = "orig.ident")
ggsave("sample4_kb/mira4_meta.pdf", width = 25, height = 50, units = c('cm'))
```
...
ggpairs(seu@meta.data, columns = cols_use,
        ...
        progress = FALSE)
ggsave("sample4_kb/mira4_corr.pdf", width = 25, height = 25, units = c('cm'))
...
DimPlot(seu, reduction = "tsne",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample4_kb/mira4_tsne.pdf", width = 15, height = 10, units = c('cm'))
...
DimPlot(seu, reduction = "umap",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample4_kb/mira4_umap.pdf", width = 15, height = 10, units = c('cm'))
...
ggsave("sample4_kb/mira4_umap1.pdf", width = 15, height = 10, units = c('cm'))
...
seu.sample4_metadata <- seu@meta.data
```

```{r}
library(tidyverse)
run1.sample4.sorted.filt_metadata <- tibble::rownames_to_column(run1.sample4.sorted.filt_metadata, "Cell.IDs")
seu.sample4_metadata <- tibble::rownames_to_column(seu.sample4_metadata, "Cell.IDs")

run1.sample4.sorted.filt_metadata$Cell.IDs <- strtrim(run1.sample4.sorted.filt_metadata$Cell.IDs, 16)
```

Combine metadat. I am going to do an inner join: only if cellID is in both metadata tables
```{r}
commonsample4 <- merge(run1.sample4.sorted.filt_metadata, seu.sample4_metadata, by = "Cell.IDs")
```

Here, I am going to make a new object with only cells in both seurat objects
```{r}
toRemove <-c(commonsample4$Cell.IDs)

## filter them out:
commCells_sample4 <- seu[,colnames(seu) %in% toRemove]

length(colnames(commCells_sample4))

commonsample4 <- commonsample4 %>%
     remove_rownames() %>%
...
```{r}
commCells_sample4@meta.data <- commonsample4
```

And now export of velocity analysis
```{r}
write.csv(x = commCells_sample4@meta.data, file = 'sample4_kb/meta_filtered_fromOG_sample4_v10.csv')
write.csv(x = t(as.matrix(commCells_sample4@assays$sf@counts)), file = 'sample4_kb/spliced_raw_filtered_fromOG_sample4_v10.csv')
write.csv(x = t(as.matrix(commCells_sample4@assays$uf@counts)), file = 'sample4_kb/unspliced_raw_filtered_fromOG_sample4_v10.csv')
```
