...
```{r}
d <- "/Users/ta13/R_git/miracidia_v10_scvelo/sample1_kb/counts_filtered/"
...
9920 genes, 3503 cells

```{r}
dim(unspliced)
```

9920 genes,3504 cells
...
```{r, include=FALSE}
saveRDS(bc2, "/Users/ta13/R_git/miracidia_v10_scvelo/sample1_kb/counts_filtered/bivar_knee.rds")
```
...
```{r}
write.csv(x = seu@meta.data, file = 'sample1_kb/seu_meta_filt_miracidia_sample1.csv')
write.csv(x = t(as.matrix(seu@assays$sf@counts)), file = 'sample1_kb/seu_spliced_raw_filt_miracidia_sample1.csv')
write.csv(x = t(as.matrix(seu@assays$uf@counts)), file = 'sample1_kb/seu_unspliced_raw_filt_miracidia_sample1.csv')
```

```{r, fig.height=12, fig.width=6}
cols_use <- c("nCount_sf", "nFeature_sf", "nCount_uf", "nFeature_uf")
VlnPlot(seu, cols_use, pt.size = 0.1, ncol = 1, group.by = "orig.ident")
ggsave("sample1_kb/mira1_meta.pdf", width = 25, height = 50, units = c('cm'))
```
...
ggpairs(seu@meta.data, columns = cols_use,
        ...
        progress = FALSE)
ggsave("sample1_kb/mira1_corr.pdf", width = 25, height = 25, units = c('cm'))
...
DimPlot(seu, reduction = "tsne",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample1_kb/mira1_tsne.pdf", width = 15, height = 10, units = c('cm'))
...
DimPlot(seu, reduction = "umap",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample1_kb/mira1_umap.pdf", width = 15, height = 10, units = c('cm'))
...
ggsave("sample1_kb/mira1_umap1.pdf", width = 15, height = 10, units = c('cm'))
...
```{r}
run1.sample1.sorted.filt_metadata <- run1.sample1.sorted.filt@meta.data

seu.sample1_metadata <- seu@meta.data
```

```{r}
library(tidyverse)
run1.sample1.sorted.filt_metadata <- tibble::rownames_to_column(run1.sample1.sorted.filt_metadata, "Cell.IDs")
seu.sample1_metadata <- tibble::rownames_to_column(seu.sample1_metadata, "Cell.IDs")

run1.sample1.sorted.filt_metadata$Cell.IDs <- strtrim(run1.sample1.sorted.filt_metadata$Cell.IDs, 16)
```

Combine metadat. I am going to do an inner join: only if cellID is in both metadata tables
```{r}
commonsample1 <- merge(run1.sample1.sorted.filt_metadata, seu.sample1_metadata, by = "Cell.IDs")
```

Here, I am going to make a new object with only cells in both seurat objects
```{r}
toRemove <-c(commonsample1$Cell.IDs)

## filter them out:
commCells_sample1 <- seu[,colnames(seu) %in% toRemove]

length(colnames(commCells_sample1))

commonsample1 <- commonsample1 %>%
     remove_rownames() %>%
...
```{r}
commCells_sample1@meta.data <- commonsample1
```

And now export of velocity analysis
```{r}
write.csv(x = commCells_sample1@meta.data, file = 'sample1_kb/meta_filtered_fromOG_sample1_v10.csv')
write.csv(x = t(as.matrix(commCells_sample1@assays$sf@counts)), file = 'sample1_kb/spliced_raw_filtered_fromOG_sample1_v10.csv')
write.csv(x = t(as.matrix(commCells_sample1@assays$uf@counts)), file = 'sample1_kb/unspliced_raw_filtered_fromOG_sample1_v10.csv')
```
