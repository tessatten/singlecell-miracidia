...
```{r}
#new_pal <- c("#9E0059","#007aaa","#ffb703","#BBC8CA","#fb8500","#7851a9","#00325b","#8ACB88","#107E7D", "#FB6376", "#c40c18", "#FFFD82", "#2191FB", "#690500", "#57467B", "#B57F50","#2C514C","#C1AAC0" ,"#FFCDB2" )
scales::show_col(new_pal)
plot1 <- DimPlot(run1.combined.v10, reduction = "umap", label = FALSE, repel = TRUE, label.box = FALSE) + NoLegend() +scale_color_manual(values = new_pal)
LabelClusters(plot1, id = "ident", color = 'black', size =4, repel = T,  box.padding = 1.75)

ggsave(paste("figs_nov23/run1.combined.v10_umap_fancylabels-small-grey",st, ".pdf", sep = ""))
```

PLot by sorted/unsorted
```{r}
...
ggsave(paste("figs_nov23/run1.combined.v10_umap_fancylabels-sortsmallish-",st, ".pdf", sep = ""), width = 28, height = 12, units = c('cm'))
```

Import the marker gene list
```{r}
marker_gene_list <- read.csv("/Users/ta13/R_git/filtered_v10_miracidia/markerz_roc_with_lbls_run1.combined.v10_jan23.csv", stringsAsFactors = FALSE, header = TRUE) #import the marker data from somule papaer
marker_gene_list_v9annotated <- read.csv("/Users/ta13/R_git/filtered_v10_miracidia/markerz_roc_with_lbls_run1.combined.v10_annotate_2023-01-23.csv", stringsAsFactors = FALSE, header = TRUE) #import the marker data from somule papae
```
...
```{r}
mira.neuron3 <- subset(run1.combined.v10, idents = 'Neuron 3')

coexpression.plot <-FeaturePlot(mira.neuron3, features = c("Smp-180030", "Smp-104210"), blend = TRUE)
coexpression.plot
ggsave(paste("figs_nov23/run1.combined.v10.neuron3-coexpressed-Smp-180030-Smp-104210-",st, ".pdf", sep = ""), width = 45, height = 25, units = c('cm'))
```
...
mira.neuron3 <- subset(run1.combined.v10, idents = c("Neuron 3"))
...
ThisWayIsTotallyMentalButItWorks <- mira.neuron3
GOI1 <- 'Smp-180030' #you will have to name your first gene here, im choosing PDX1 as an example
GOI2 <- 'Smp-104210' #you will have to name your second gene here, im choosing INS as an example
...
```{r}
FeaturePlot(run1.combined.v10, features = "Smp-041540")
ggsave(paste("figs_nov23/run1.combined.v10-Smp-041540-",st, ".pdf", sep = ""), width = 25, height = 15, units = c('cm'))
...
```{r}
deg_genelist <- read.csv(("/Users/ta13/R_git/filtered_v10_miracidia/top25stemDEGs.csv"), stringsAsFactors = FALSE, header = TRUE) #import the marker data from somule papaer
...
DefaultAssay(stems) <- "RNA"
stems <- ScaleData(stems, verbose = FALSE, vars.to.regress = c("percent.mt", "plex"))
