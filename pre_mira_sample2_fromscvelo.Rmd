...
```{r}
d <- "/Users/ta13/R_git/miracidia_v10_scvelo/sample2_kb/counts_filtered/"
...
```{r, include=FALSE}
saveRDS(bc2, "/Users/ta13/R_git/miracidia_v10_scvelo/sample2_kb/counts_filtered/bivar_knee.rds")
```
...
```{r}
write.csv(x = seu@meta.data, file = 'sample2_kb/seu_meta_filt_miracidia_sample2.csv')
write.csv(x = t(as.matrix(seu@assays$sf@counts)), file = 'sample2_kb/seu_spliced_raw_filt_miracidia_sample2.csv')
write.csv(x = t(as.matrix(seu@assays$uf@counts)), file = 'sample2_kb/seu_unspliced_raw_filt_miracidia_sample2.csv')
```

```{r, fig.height=12, fig.width=6}
cols_use <- c("nCount_sf", "nFeature_sf", "nCount_uf", "nFeature_uf")
VlnPlot(seu, cols_use, pt.size = 0.1, ncol = 1, group.by = "orig.ident")
ggsave("sample2_kb/mira2_meta.pdf", width = 25, height = 50, units = c('cm'))
```
...
ggpairs(seu@meta.data, columns = cols_use,
        ...
        progress = FALSE)
ggsave("sample2_kb/mira2_corr.pdf", width = 25, height = 25, units = c('cm'))
...
DimPlot(seu, reduction = "tsne",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample2_kb/mira2_tsne.pdf", width = 15, height = 10, units = c('cm'))
...
DimPlot(seu, reduction = "umap",
        group.by = "orig.ident", pt.size = 0.5, label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
ggsave("sample2_kb/mira2_umap.pdf", width = 15, height = 10, units = c('cm'))
...
ggsave("sample2_kb/mira2_umap1.pdf", width = 15, height = 10, units = c('cm'))
...
```{r}
run1.sample2.sorted.filt <- readRDS(file = "/Users/ta13/R_git/filtered_v10_miracidia/run1.v10.sample2.unsorted.filt_52.rds")
```

Extract list with metadata
```{r}
run1.sample2.sorted.filt_metadata <- run1.sample2.sorted.filt@meta.data

seu.sample2_metadata <- seu@meta.data
```

```{r}
library(tidyverse)
run1.sample2.sorted.filt_metadata <- tibble::rownames_to_column(run1.sample2.sorted.filt_metadata, "Cell.IDs")
seu.sample2_metadata <- tibble::rownames_to_column(seu.sample2_metadata, "Cell.IDs")

run1.sample2.sorted.filt_metadata$Cell.IDs <- strtrim(run1.sample2.sorted.filt_metadata$Cell.IDs, 16)
```

Combine metadat. I am going to do an inner join: only if cellID is in both metadata tables
```{r}
commonsample2 <- merge(run1.sample2.sorted.filt_metadata, seu.sample2_metadata, by = "Cell.IDs")
```

Here, I am going to make a new object with only cells in both seurat objects
```{r}
toRemove <-c(commonsample2$Cell.IDs)

## filter them out:
commCells_sample2 <- seu[,colnames(seu) %in% toRemove]

length(colnames(commCells_sample2))

commonsample2 <- commonsample2 %>%
     remove_rownames() %>%
...
```{r}
commCells_sample2@meta.data <- commonsample2
```

And now export of velocity analysis
```{r}
write.csv(x = commCells_sample2@meta.data, file = 'sample2_kb/meta_filtered_fromOG_sample2_v10.csv')
write.csv(x = t(as.matrix(commCells_sample2@assays$sf@counts)), file = 'sample2_kb/spliced_raw_filtered_fromOG_sample2_v10.csv')
write.csv(x = t(as.matrix(commCells_sample2@assays$uf@counts)), file = 'sample2_kb/unspliced_raw_filtered_fromOG_sample2_v10.csv')
```
