...
First I am going to look at sample 1 from MIRACIDIA run 1

```{r}
run1.v10.sample1.sorted.filt <- readRDS(file = "/Users/ta13/R_tutorials/neat_miracidia/run1.v10.sample1.sorted.filt.rds")

write.csv(Cells(run1.v10.sample1.sorted.filt), file = "run1.v10.sample1.sorted.filt_cellID_obs.csv", row.names = FALSE)

write.csv(Embeddings(run1.v10.sample1.sorted.filt, reduction = "umap"), file = "run1.v10.sample1.sorted.filt_cell_embeddings.csv")

write.csv(run1.v10.sample1.sorted.filt@meta.data$SCT_snn_res.0.8, file = "run1.v10.sample1.sorted.filt_clusters.csv")
```


```{r}
splice <- read.csv("/Users/ta13/R_git/mira_phate/seu_meta.csv", stringsAsFactors = FALSE, header = TRUE) #import the marker data from somule papaer
...
```{r}
mira_markers <- read.csv("/Users/ta13/R_git/v10_miracidia/mira_topGO/prepsctmarkersmarker_roc_run1.combined.v10_dec19-stem.csv", stringsAsFactors = FALSE, header = TRUE) #import the marker data from somule papaer
mira_markers$gene <- gsub('\\-', '_', mira_markers$gene) #replace dashes in geneid with underscores
write.csv(x=mira_markers, file=('/Users/ta13/R_git/v10_miracidia/mira_topGO/prepsctmarkersmarker_roc_run1.combined.v10_dec19-stem-gene.csv'), row.names = FALSE) #save this as a csv
